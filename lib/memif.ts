...
function newNativeMemif(opts: NativeMemifOptions): NativeMemif {
  ...
  try {
    addon = createRequire(import.meta.url)("../build/Release/memif-native");
  } catch (err: unknown) {
    ...
    } else if (!fs.existsSync(path.join(import.meta.dirname, "../build/Release/memif-native.node"))) {
